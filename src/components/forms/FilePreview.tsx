...
export default function FilePreview({ deleteFile, file, readOnly }: FilePreviewProps): React.ReactElement {
    ...
    return imagesType.includes(file.type) ? (
        <>
            <li className='flex items-center justify-between py-3 pl-3 pr-4 text-sm' key={file.name}>
                <div className='flex w-0 flex-1 items-center'>
                    <HiOutlinePhotograph className='h-5 w-5 flex-shrink-0 text-gray-400' aria-hidden='true' />
            ...
            {isOpen && (
                <Lightbox
                    ...
                    onMovePrevRequest={() => setIndex((prevIndex) => (prevIndex + images.length - 1) % images.length)}
        ...
        <li key={file.name} className='flex items-center justify-between py-3 pl-3 pr-4 text-sm'>
            <div className='flex w-0 flex-1 items-center'>
                <HiOutlinePaperClip className='h-5 w-5 flex-shrink-0 text-gray-400' aria-hidden='true' />
                <span className='ml-2 w-0 flex-1 truncate'>{file.name}</span>
            </div>
            <div className='ml-4 flex flex-shrink-0 items-center space-x-2'>
                <UnstyledLink
